...
import { auth, getUserInfo } from '../(firebasethings)/firebase';
import { onAuthStateChanged } from 'firebase/auth';
import User from './user';

export default function IndexPage({ searchParams }) {
...
<NavItem href="/">
  South Africa Stock Exchange
                  ...
                  {showDropdown && (
                    <div id="dropdown" className="z-10 bg-transparent  rounded-lg ">
                      <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                        <li>
                          <Link href="/Forex" className=" text-slate-300 block px-4 py-2 hover:text-white 00 ">Forex</Link>
                        </li>
                        <li>
                          <Link  href="/Forex-emerging-currencies" className=" text-slate-300 block px-4 py-2 hover:text-white 00 ">Forex (emerging currencies)</Link>
                        </li>
          ...
          <div className="flex flex-col">
            ...
            <main className="flex flex-1 flex-col p-4 md:p-6" style={{ backgroundColor: "#1A1E26", borderColor: "#FFFF" }}>
              <div className="flex items-center mb-8">
                <h1 className="font-semibold text-lg md:text-2xl text-white mt-10">{username}</h1>
              </div>
